...
Open Palace 用确定性工程解决这些问题：存算分离、代码级 PostHook、三级索引、温度模型记忆衰减、写前验证、Git 版本控制、自动会话上下文注入。

---

## 核心亮点

**Session Guard — 自动上下文注入。** Agent 在任意会话中首次调用任何 Open Palace 工具时，L0 总目录和工作状态快照会自动注入到响应中。这是代码级保证：即使 Agent 跳过启动仪式，首次接触就获得完整的全局感知。配合 `mp_session_start`（单次调用加载 index + snapshot + scratch），上下文恢复既无摩擦又不可避免。
...
10. **代码级保证。** PostHook 引擎在每次写操作后自动 git commit、更新索引、写入日志、触发搜索 reindex。Session Guard 在首次工具调用时自动注入启动上下文。管道执行，不靠 prompt 指令。
...
```
mp_session_start      → 应返回 L0 Master Index + 快照 + 最近 scratch 笔记
...
npx tsx src/test-e2e.ts   # 运行 E2E 测试（139 个断言）
```
...
- **v0.4.2** -- Session Guard（首次工具调用自动注入上下文）、`mp_session_start` 单一启动工具、MCP Resource/Prompt 注册、Cursor Rule 精简化
